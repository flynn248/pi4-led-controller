...
public static class EffectParameterErrors
{
    ...
    public static Error InvalidLength(int length) => new Error($"Effect parameter cannot exceed {length} characters").Validation(InvalidLengthErrorCode);
    public static Error InvalidFormat => new Error("Effect parameter is in an invalid format").Validation(InvalidFormatErrorCode);
}
